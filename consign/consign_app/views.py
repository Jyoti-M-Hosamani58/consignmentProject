...
def branchconsignment_edit(request, pk):
    userdata = AddConsignment.objects.filter(track_id=pk).first()  # Retrieve the consignment data or None if not found


    # Fetch all products related to this track_id
    products = AddConsignment.objects.filter(track_id=userdata)
